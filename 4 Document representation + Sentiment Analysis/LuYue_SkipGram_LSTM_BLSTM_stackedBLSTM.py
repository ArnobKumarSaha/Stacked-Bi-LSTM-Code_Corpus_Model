# -*- coding: utf-8 -*-
"""
Baseline Skip-Gram + LSTM & Bi-LSTM & stacked Bi-LSTM: 
    Train and test baseline Skip-Gram+LSTM & Skip-Gram+Bi-LSTM & Skip-Gram+stacked Bi-LSTM
...
def allInput(sentiment_list,sentence_list,raw_word_list):
    ...
    for i in range(len(sentence_list)):
        if len(sentence_list[i])>sentLength:
            # 1. 评论单词数>平均单词数 直接截取前K个单词，一条评论超出平均单词数后的单词不做处理
